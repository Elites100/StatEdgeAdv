...
export const SidebarProvider = ({ children, defaultOpen = true, className, style, ...props }) => {
  ...
  return (
    <SidebarContext.Provider value={value}>
      <div className={cn("group/sidebar-wrapper flex min-h-svh w-full has-[[data-variant=inset]]:bg-sidebar", className)} style={style} {...props}>
        {children}
      </div>
...
export const SidebarRail = React.forwardRef(({ className, ...props }, ref) => {
  ...
  return (
    <button
      ...
      className={cn("absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] hover:after:bg-sidebar-border sm:flex", className)}
      {...props}
    />
  );
});
...
export const SidebarContent = React.forwardRef(({ className, ...props }, ref) => (
  <div ref={ref} data-sidebar="content" className={cn("flex h-full w-full flex-col bg-sidebar px-2 py-2", className)} {...props} />
));
  ...
  if (tooltip) {
    ...
    return (
      <Tooltip>
        ...
        <TooltipContent side="right" align="center" {...(typeof tooltip === "object" ? tooltip : {})}>
          {content}
        </TooltipContent>
      </Tooltip>
    );
